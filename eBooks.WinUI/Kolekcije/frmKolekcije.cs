...
{
    public partial class frmKolekcije : Form
    {
        ...
        private void dgvKolekcije_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (dgvKolekcije.CurrentRow != null)
            {
                var id = dgvKolekcije.SelectedRows[0].Cells[0].Value.ToString();

                frmKolekcijeEdit frm = new frmKolekcijeEdit(int.Parse(id));
                frm.Show();
            }
        }
