...
{
    public partial class frmAutoriPretraga : Form
    {
        private readonly APIService _autoriService = new APIService("autori");
        public Model.Autori odabraniAutor { get; set; }

        public frmAutoriPretraga()
        {
            InitializeComponent();
        ...
        private async void dgvAutori_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (dgvAutori.CurrentRow != null)
            {
                var id = dgvAutori.SelectedRows[0].Cells[0].Value.ToString();

                odabraniAutor = await _autoriService.GetById<Model.Autori>(id);
